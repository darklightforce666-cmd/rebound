...
Existing-coin enrollment is deliberately not offered by the browser. The program has an independently coauthorized registration path, but an operator must first prove current authority, mutable versus finalized sharing state, original attribution, an activation slot, and unambiguous fee provenance. A finalized incompatible fee split cannot be promised to change. Existing enrollment requires additional end-to-end tests before production support is claimed. The cloned-program suite now also executes graduation before sharing setup and collects the isolated initial AMM fees afterward.
...
Observed successful full run: [34991442222](https://github.com/darklightforce666-cmd/rebound/actions/runs/34991442222), source `d471894ae494860b84fb7453c21056cfa95d2590`. This includes atomic preparation/creation, normal and early graduation, the compiled program, database permissions, and independent backend replay. The replay matches captured costs and fees, distinguishes rent and protocol-fee carving, and verifies first-vault and AMM-conversion attribution. Check the latest branch workflow for all later changes; an earlier run does not certify newer code. Windows in this workspace blocks native build subprocesses, so SBF and browser bundling run in Linux CI. Local policy and PostgreSQL WASM-engine tests are real executions of those layers.
...
Use managed PostgreSQL with TLS, backups, point-in-time recovery, and separate service credentials. Run migrations as the schema owner, then apply `scripts/rewards/database-roles.sql` and grant each NOLOGIN role to its own managed login. Production preflight rejects runtime credentials with deployment, raw-evidence, service-registry or correction write powers. The schema owner must never be a runtime credential. Then:
...
Activation evidence must include independently reviewable passing evidence for curve and graduated lifecycle, receipt attribution, full historical replay, adversarial program tests, independent security review, funding-policy calibration, and governance/timelock configuration. Start with `rewards-activation.example.json`, whose checks are all false. A file claiming these passed is operator attestation; it does not replace the underlying work. Workers can mount it using `REWARDS_ACTIVATION_FILE`; Netlify Functions can receive the same public JSON through `REWARDS_ACTIVATION_JSON`. Preflight rejects absent or mismatched fields. It checks the hash of deployed ProgramData bytes including padding, which must be recorded after deployment and compared with the reviewed build.

Production remains blocked pending those inputs and review. Additional material validation remains: real complete launch-to-current history at production throughput; worker/verifier latency within the slot limits; full worker/API/browser end-to-end execution against deployed services; governance operation; existing-coin enrollment; supported router/intermediary conversion coverage; per-lot correction accounting; and broad economic manipulation/funding calibration. The initial full-block replay approach is deliberately conservative and has not been demonstrated to scale to a busy production launchpad. Funding history is reread independently rather than trusting publisher audit caches; older purchases can require repeated archival reads. Do not relax completeness or freshness limits to make a slow worker appear ready.
