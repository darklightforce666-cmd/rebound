...
function tokenChart(mint){return window.ReboundCharts.markup(mint,mint===C.mint?'rebound':'Token');}
...
function analytics(){return heading('Creator fees & rewards','Verified reserves, conditional allocations and completed payments.')+rewardNotice()+tokenChart(C.mint);}
...
function token(mint){
 ...
 return heading('Token overview','Mainnet token verification and indexed market data.')+'<section class="card live-card"><div class="section-heading"><h2 id="token-name">'+(mint===C.mint?'Configured token':'Solana token')+'</h2>'+external('https://solscan.io/token/'+mint,'View on Solscan')+'</div><p class="live-address">'+esc(mint)+'</p><div id="mint-status" aria-live="polite">Checking mainnet…</div></section><section class="card live-card"><div id="token-market" aria-live="polite">Loading market data…</div></section>'+tokenChart(mint)+rewardNotice()+lookup();
}
...
function render(){
 ...
 if(route==='explore'){loadMint(C.mint,'#configured-status','#configured-name',id,signal);loadMarket(C.mint,'#configured-market',id,signal);}
