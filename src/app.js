...
function rewardNotice(){return '<section class="card live-card"><span class="tag neutral">REWARDS NOT ACTIVE</span><h2>Creator fees to holder rewards</h2><p>The intended allocation is 85% of collected creator fees for holders and 15% for operations. Collection, the reward vault, and payout processing must be connected before rewards can be paid.</p><a href="#docs" class="btn outline small">View launch status</a></section>';}
...
function explore(){return '<section class="hero live-hero"><div class="hero-copy"><h1>Launch a token.<br><em>Reward topblasters.</em></h1><p>A launchpad built around Pump.fun.<br>Creator fees will fund SOL rewards for holders.</p><div class="hero-actions"><a href="#launch" class="btn">Launch status ↗</a><a href="#docs" class="btn ghost">How it works</a></div><div class="hero-fineprint">Mainnet data · Creator fee rewards in preparation</div></div><div class="live-hero-mark"><img src="assets/rebound-logo.png" alt="rebound logo"><span>85% holders · 15% operations</span></div></section>'+lookup()+tokenCard(C.mint)+solChart();}
function launch(){return heading('Launch through Pump.fun','Creator fees are the source of REBOUND rewards.')+'<section class="card live-card"><span class="tag neutral">LAUNCHES NOT AVAILABLE YET</span><h2>Finish the fee connection first</h2><p>Launching through REBOUND will create a Pump.fun token and connect its creator fees to the reward system. A configured mint address alone does not create a token or enable payouts.</p><div class="live-steps"><article><span>01</span><h3>Create the token</h3><p>Pump.fun creates the mint and bonding curve. No separate liquidity pool is needed to start.</p></article><article><span>02</span><h3>Route creator fees</h3><p>The launch must use the correct fee recipient. Fees collected before and after graduation must reach the token\'s reward vault.</p></article><article><span>03</span><h3>Activate rewards</h3><p>The deployed reward program and payout service must be connected and verified.</p></article></div><button class="btn" disabled>Launch unavailable</button><p class="live-caption">No token is created and no payment is requested on this page.</p></section>'+rewardNotice();}
