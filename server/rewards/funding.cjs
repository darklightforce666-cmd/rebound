...
async function refresh({db,rpc,coin,replay,cutoff}){
 const deployment=(await db.query('SELECT genesis FROM reward_deployments WHERE id=$1',[coin.deployment])).rows[0];
 if(!deployment||await rpc.call('getGenesisHash',[])!==deployment.genesis)throw Error('Funding history network mismatch');
 const corrections=(await db.query('SELECT edge_id FROM reward_link_corrections WHERE mint=$1',[coin.mint])).rows;
 const revoked=new Set(corrections.map(c=>c.edge_id)),freshLinks=[];
 ...
 for(const c of (await db.query('SELECT c.mint,c.treasury,d.program FROM reward_coins c JOIN reward_deployments d ON d.id=c.deployment WHERE c.deployment=$1',[coin.deployment])).rows)if(c.treasury===W.addresses(c.program,c.mint).coin.toBase58())services.push({address:c.treasury,classification:'rebound-payout',verified:true,evidence:{deployment:coin.deployment,source:'derived-program-PDA'}});
 const results=[];
 for(const wallet of [...new Set(replay.lots.map(l=>l.wallet))]){
  ...
  for(const lot of replay.lots.filter(l=>l.wallet===wallet)){
   ...
   // Publisher audit rows are evidence, never trusted cached decisions.
   // Independently reread archive transactions before verification.
   let result;
   {let purchaseHistory=history;if(history.complete&&history.startTime>lot.at-P.POLICY.fundingHistorySeconds)purchaseHistory=await H.walletHistory(rpc,wallet,{cutoffSlot:lot.slot,cutoffTime:lot.at});
   ...
   for(const edge of [...result.edges,...result.holds]){
    ...
    if(!revoked.has(id))freshLinks.push({...edge,id,mint:coin.mint,status:edge.status||'ambiguous'});
 ...
 }return{wallets:results,links:freshLinks};
}
