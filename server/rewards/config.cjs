...
async function preflight(connection,db,cfg=settings()){
 const blockers=[];for(const k of ['program','operations','publisher','verifier','guardian','rpc','historyRpc','activationFile'])if(!cfg[k])blockers.push(k+' not configured');
 ...
 try{
  ...
  const upgrade=W.pk(pd.data.subarray(13,45)).toBase58(),activation=JSON.parse(await fs.readFile(cfg.activationFile,'utf8'));
